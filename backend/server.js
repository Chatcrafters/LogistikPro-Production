// backend/server.js - KOMPLETT NEU GESCHRIEBEN
// ✅ Alle Features erhalten + Milestone-System repariert
...
// ✅ REPARIERTE CORS-Konfiguration für Frontend
app.use(cors({
  origin: [
...
// ✅ Zusätzlicher CORS-Handler für alle Preflight-Requests
app.use((req, res, next) => {
  ...
  if (req.method === 'OPTIONS') {
    console.log('✅ CORS Preflight für:', req.path);
    res.sendStatus(200);
  } else {
    next();
  }
...
// ================================
// 🚦 MILESTONE SYSTEM - REPARIERT!
// ================================

// ✅ MILESTONE-SYSTEM MIT SEPARATEN AMPEL-SPEICHERUNGEN
app.put('/api/shipments/:id/milestone', async (req, res) => {
  try {
    const { id } = req.params;
    const { milestone, ampelType } = req.body;
    
    console.log(`🎯 SEPARATE AMPEL-MILESTONE: Sendung ${id}, Ampel ${ampelType} → Milestone ${milestone}`);

    if (!milestone || milestone < 0 || milestone > 10) {
    ...
    const milestoneTexts = {
      0: "Nicht begonnen",
      1: "Abholung beauftragt", 2: "Sendung abgeholt", 3: "Anlieferung im Lager",
      4: "Sendung gebucht", 5: "Zoll erledigt", 6: "Anlieferung bei Airline", 
      7: "Sendung abgeflogen", 8: "Sendung angekommen", 9: "Sendung verzollt", 10: "Sendung zugestellt"
    };

    // ✅ SCHRITT 1: Aktuelle Sendung laden mit ALLEN Milestone-Feldern
    const { data: currentShipment, error: readError } = await supabase
      .from('shipments')
      .select('pickup_milestone, carrier_milestone, delivery_milestone, completed_milestones, milestone_history, current_milestone')
      .eq('id', parseInt(id))
      .single();

    if (readError) {
      console.error('❌ Sendung nicht gefunden:', readError);
      return res.status(404).json({ error: 'Sendung nicht gefunden', success: false });
    }

    // ✅ SCHRITT 2: Bestimme welche Ampel aktualisiert wird
    let updateData = {};
    
    // Bestimme Ampel-Typ basierend auf Milestone-Nummer ODER explizitem ampelType
    let detectedAmpelType = ampelType;
    if (!detectedAmpelType) {
      if (milestone >= 1 && milestone <= 3) detectedAmpelType = 'abholung';
      else if (milestone >= 4 && milestone <= 8) detectedAmpelType = 'carrier';
      else if (milestone >= 9 && milestone <= 10) detectedAmpelType = 'zustellung';
    }

    console.log(`🚦 Erkannte Ampel: ${detectedAmpelType} für Milestone ${milestone}`);

    // ✅ SCHRITT 3: Setze die RICHTIGE Ampel-Spalte
    switch(detectedAmpelType) {
      case 'abholung':
        updateData.pickup_milestone = milestone;
        break;
      case 'carrier':
        updateData.carrier_milestone = milestone;
        break;
      case 'zustellung':
        updateData.delivery_milestone = milestone;
        break;
      default:
        console.warn('⚠️ Unbekannter Ampel-Typ:', detectedAmpelType);
        updateData.current_milestone = milestone; // Fallback
    }

    // ✅ SCHRITT 4: Completed Milestones Array verwalten
    let completedMilestones = currentShipment.completed_milestones || [];
    let milestoneHistory = currentShipment.milestone_history || [];

    if (milestone > 0 && !completedMilestones.includes(milestone)) {
      completedMilestones.push(milestone);
      completedMilestones.sort((a, b) => a - b);

      // Historie-Eintrag
      const now = new Date();
      const historyEntry = {
        milestone: milestone,
        text: milestoneTexts[milestone],
        ampel: detectedAmpelType,
      ...
      milestoneHistory.push(historyEntry);
      milestoneHistory.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    }

    // ✅ SCHRITT 5: Höchsten Milestone als current_milestone (Kompatibilität)
    const newCurrentMilestone = completedMilestones.length > 0 ? Math.max(...completedMilestones) : 0;

    // ✅ SCHRITT 6: Alle Updates zusammenführen
    updateData = {
      ...updateData,
    ...
    if (updateError) {
      ...
      return res.status(500).json({ error: 'Datenbankfehler', success: false });
    }

    console.log('✅ SEPARATE AMPEL-MILESTONE GESPEICHERT:', {
      ampel: detectedAmpelType,
      milestone,
      pickup_milestone: updatedShipment.pickup_milestone,
      carrier_milestone: updatedShipment.carrier_milestone,
      delivery_milestone: updatedShipment.delivery_milestone
    });
    
    res.json({
      success: true,
      data: updatedShipment,
      message: `Milestone ${milestone} für Ampel ${detectedAmpelType} gespeichert`,
      milestone_text: milestoneTexts[milestone],
      ampel_milestones: {
        pickup: updatedShipment.pickup_milestone || 0,
        carrier: updatedShipment.carrier_milestone || 0,
        delivery: updatedShipment.delivery_milestone || 0
      }
    });

  } catch (error) {
    console.error('❌ Milestone Ampel Fehler:', error);
    res.status(500).json({ error: 'Server-Fehler', success: false });
  }
});
...
// GET /api/partners/:partnerId/rates - Tarife für einen Partner abrufen
app.get('/api/partners/:partnerId/rates', async (req, res) => {
  try {
    const { partnerId } = req.params;
    const { absenderPlz, weight, zone } = req.query;
    
    console.log('🔍 TARIF-ANFRAGE:', { 
      partnerId, 
      absenderPlz, 
    ...
    // Zone ermitteln basierend auf Partner und PLZ
    let zoneCode = zone;
    
    if (!zoneCode && absenderPlz) {
      // Bestimme welche Tabelle basierend auf Partner
      let zoneTable = '';
      
      if (partnerId == 1) {
        zoneTable = 'hut_postal_zones';
      } else if (partnerId == 3 || partnerId == 23) {
        zoneTable = 'bat_postal_zones';
      }
      ...
      if (zoneTable) {
        try {
          const { data: zoneData, error: zoneError } = await supabase
            .from(zoneTable)
            .select('zone_code')
            .or(`postal_code.eq.${absenderPlz},postal_code.eq.${absenderPlz.substring(0,3)},postal_code.eq.${absenderPlz.substring(0,2)}`)
            .limit(1);
          
          if (!zoneError && zoneData && zoneData.length > 0) {
            zoneCode = zoneData[0].zone_code;
            console.log('✅ Zone gefunden:', zoneCode);
          }
        } catch (zoneError) {
          console.error('❌ Fehler bei Zone-Ermittlung:', zoneError);
        }
      }
    }
    
    // Fallback auf ZONE1 wenn keine Zone gefunden
    if (!zoneCode) {
      zoneCode = 'ZONE1';
      console.log('⚠️ Verwende Default-Zone:', zoneCode);
    }

    // Tarif abrufen mit korrekten Spaltennamen
    ...
    const { data: rates, error: rateError } = await supabase
      ...
      .lte('weight_from', weightNum)
      .gte('weight_to', weightNum)
      .order('weight_from', { ascending: false })
      .limit(1);

    console.log('📊 Query-Ergebnis:', rates?.length || 0, 'Einträge gefunden');
    
    if (rateError) {
      console.error('❌ Datenbank-Fehler:', rateError);
    ...
    if (!rates || rates.length === 0) {
      // Versuche nächsthöheren Tarif zu finden
      const { data: fallbackRates, error: fallbackError } = await supabase
        .from('partner_base_rates')
        .select('*')
        .eq('partner_id', parseInt(partnerId))
        .eq('zone_code', zoneCode)
        .gt('weight_from', weightNum)
        .order('weight_from', { ascending: true })
        .limit(1);
      
      if (!fallbackError && fallbackRates && fallbackRates.length > 0) {
        console.log('⚠️ Verwende nächsthöheren Tarif');
        const rate = fallbackRates[0];
        
        return res.json({
          success: true,
          rate: {
            basePrice: parseFloat(rate.base_price),
            xrayBase: parseFloat(rate.xray_base) || 0,
            xrayPerUnit: parseFloat(rate.xray_per_unit) || 0,
            xrayUnitType: rate.xray_unit_type || 'per_kg',
            xrayIncludedUnits: parseInt(rate.xray_included_units) || 0,
            zone: rate.zone_code,
            weightFrom: parseFloat(rate.weight_from),
            weightTo: parseFloat(rate.weight_to),
            warning: 'Nächsthöherer Tarif verwendet'
          }
        });
      }
      
      console.log('❌ Kein Tarif gefunden für:', { 
    ...
    const rate = rates[0];
    console.log('✅ Tarif gefunden:', rate);
    ...
    res.status(500).json({ 
      error: 'Serverfehler beim Abrufen der Tarife',
...
app.listen(PORT, () => {
  ...
  console.log(`🚦 Milestone-System: REPARIERT!`);
});