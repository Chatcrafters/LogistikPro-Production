...
const NeueSendungSuper = ({ onClose, onSave }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    abholort: '',
    referenz: '',
    gewichtsModus: 'einzeln', // 'einzeln' oder 'gesamt'
    
    // 🚀 NEUE PARTNER-FELDER
    vorlaufPartner: '',
    hauptlaufPartner: [], // Array für Multi-Select
    zustellungPartner: '',
  ...
  { 
    ...
    hoehe: '', 
    gewichtProStueck: '', 
    gewichtTyp: 'proStueck', // STELLE SICHER DASS DIES HIER IST
    volumen: 0 
  }
],
  ...
  const loadMasterData = async () => {
    ...
    // 🚀 ERWEITERTE PARTNER-STRUKTURIERUNG
    const structuredPartners = {
      raw: partnersData // Wichtig: Alle Partner für intelligente Auswahl
    };
    
    setPartners(structuredPartners);
    
    // Partner formatieren für PartnerKalkulation
    const formattedPartners = {
      abholung: partnersData.filter(p => p.type === 'carrier').map(p => p.name),
      hauptlauf: {
        luftfracht: partnersData.filter(p => p.type === 'carrier').map(p => p.name),
        seefracht: partnersData.filter(p => p.type === 'carrier').map(p => p.name),
        lkw: partnersData.filter(p => p.type === 'carrier').map(p => p.name)
      },
      zustellung: {
        USA: partnersData.filter(p => p.type === 'carrier').map(p => p.name),
        EU: partnersData.filter(p => p.type === 'carrier').map(p => p.name),
        Asien: partnersData.filter(p => p.type === 'carrier').map(p => p.name)
      }
    };
    
    // Speichere Partner für PartnerKalkulation
    window.partnerListen = formattedPartners;
...
const getCountryFromPartner = (partner) => {
  // Versuche Land aus Partner-Namen oder Adresse zu ermitteln
...
const processFile = async (file) => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (file.type === 'application/pdf') {
          ...
          // Automatisch zu Text-Modus wechseln
    ...
    if (file.type.startsWith('image/')) {
      ...
      reader.readAsArrayBuffer(file); // PDF als ArrayBuffer lesen
    } else {
...
const processPDFContent = async (content) => {
  ...
  // Da wir PDF.js nicht installiert haben, verwenden wir eine vereinfachte Lösung
  // Für eine echte PDF-Extraktion würde man PDF.js verwenden
  
  // Simuliere bekannte PDF-Inhalte basierend auf dem Dateinamen
  const extractedData = [];
  
  // Versuche, den Dateinamen zu analysieren
  if (content.includes('dangerous') || content.includes('Dangerous')) {
    extractedData.push({ field: 'Sonderanweisungen', value: 'DGR/Gefahrgut' });
    extractedData.push({ field: 'Warenart', value: 'Dangerous Goods' });
  }
  
  if (content.includes('invoice') || content.includes('Invoice')) {
    extractedData.push({ field: 'Dokumenttyp', value: 'Commercial Invoice' });
  }
  
  // Für echte PDF-Verarbeitung würde hier PDF.js verwendet:
  /*
  try {
    const pdfjsLib = await import('https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js');
    const pdf = await pdfjsLib.getDocument({ data: atob(content.split(',')[1]) }).promise;
    const page = await pdf.getPage(1);
    const textContent = await page.getTextContent();
    const text = textContent.items.map(item => item.str).join(' ');
    
    // Dann würde hier die normale Text-Verarbeitung stattfinden
  ...
  */
  ...
  // Fallback: Informiere User über PDF-Limitation
  extractedData.push({ 
    field: 'Hinweis', 
    value: 'PDF-Text-Extraktion begrenzt. Bitte kopiere relevanten Text manuell in das Text-Feld.' 
  });
  
  return extractedData;
};

const processImageContent = async (content) => {
  // Placeholder für OCR
  // Hier könnte Tesseract.js integriert werden
  return [
    { field: 'Hinweis', value: 'Bild-OCR in Entwicklung' }
  ];
};
...
const processTextContent = async (content) => {
  ...
  // Verwende die gleiche Magic Input Logic
  console.log('📄 Processing text content:', content.substring(0, 200));
  
  // 1. PACKSTÜCKE ERKENNEN
...
const applyExtractedData = () => {
  // Wende extrahierte Daten auf das Formular an
  ...
  // Setze Standard gewichtTyp wenn nicht vorhanden
  if (!newColli[index].gewichtTyp) {
    newColli[index].gewichtTyp = 'proStueck';
  }
  
  // Volumen berechnen wenn alle Maße vorhanden
  if (field === 'laenge' || field === 'breite' || field === 'hoehe') {
    ...
    newColli[index].volumen = ((l * b * h) / 1000000).toFixed(3); // in m³
    
    // Volumengewicht berechnen (für Luftfracht)
    newColli[index].volumengewicht = ((l * b * h) / 6000).toFixed(1); // in kg
  }
  
  // Gewicht pro Stück berechnen wenn "gesamt" gewählt
  if (field === 'gewichtProStueck' || field === 'gewichtTyp' || field === 'anzahl') {
    ...
    if (newColli[index].gewichtTyp === 'gesamt' && anzahl > 0) {
      // Internes gewichtProStueck ist immer pro Stück für die Berechnung
  ...
  setFormData(prev => ({
    ...
    colli: [...prev.colli, { 
      ...
      hoehe: '', 
      gewichtProStueck: '', 
      gewichtTyp: 'proStueck',  // <-- NEU HINZUGEFÜGT!
      volumen: 0 
    }]
  }));
};
...
const calculateTotals = () => {
  ...
  const totalGewicht = formData.colli.reduce((sum, c) => {
    ...
    // WICHTIG: Default auf 'proStueck' wenn nicht gesetzt
    const gewichtTyp = c.gewichtTyp || 'proStueck';
    
    if (gewichtTyp === 'gesamt') {
      return sum + gewicht; // Gesamtgewicht direkt addieren
    } else {
      return sum + (anzahl * gewicht); // Pro Stück multiplizieren
    }
  }, 0);
  ...
  const sendungData = {
    ...
    customer_id: selectedKunde?.id, // Doppelt für Kompatibilität
    abholort_id: selectedAbholort?.id,
    abholAdresseId: selectedAbholort?.id, // Doppelt für Kompatibilität
    pickup_address_id: selectedAbholort?.id,
    
    // Flughäfen - WICHTIG: Beide Varianten setzen!
    vonFlughafen: formData.vonFlughafen,
    nachFlughafen: formData.nachFlughafen,
    from_airport: formData.vonFlughafen,  // Füge diese hinzu!
    to_airport: formData.nachFlughafen,   // Füge diese hinzu!
    
    // Datum
    abholDatum: formData.abholDatum,
    pickup_date: formData.abholDatum,  // Füge diese hinzu!
    
    // Totale
    position: `SND-${new Date().getFullYear()}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`,
  ...
  console.log('=== NeueSendungSuper handleSubmit ===');
  console.log('selectedKunde:', selectedKunde);
  console.log('selectedAbholort:', selectedAbholort);
  console.log('sendungData:', sendungData);
  console.log('=== GEWICHTE IN SENDUNGDATA ===');
  console.log('gesamtGewicht:', sendungData.gesamtGewicht);
  console.log('gesamtVolumen:', sendungData.gesamtVolumen);
  console.log('gesamtColli:', sendungData.gesamtColli);
  ...
  // Hole Abholorte für gewählten Kunden
  const selectedKunde = kunden.find(k => k.name === formData.kunde);
...
<div style={{ backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
      ...
      <select 
        ...
        onChange={(e) => {
          ...
          setFormData(prev => ({ 
            ...
            // Auto-reset Partner bei Änderung
            vorlaufPartner: '',
            hauptlaufPartner: [],
            zustellungPartner: ''
          }));
          // Trigger intelligent defaults
          updateIntelligentDefaults({ ...formData, transportArt: newValue });
        }}
      >
        <option value="luftfracht-teile">🔧 Luftfracht (Teile)</option>
  ...
  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
    <div>
      ...
      <input 
        ...
        onChange={(e) => {
          ...
          // Trigger intelligent defaults after airport change
    ...
    <div>
      <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '4px' }}>
        Nach {formData.transportArt === 'luftfracht' ? 'Flughafen' : formData.transportArt === 'seefracht' ? 'Hafen' : 'Ort'}
      </label>
      <input 
        type="text"
        style={{ width: '100%', padding: '8px', border: '1px solid #d1d5db', borderRadius: '4px' }}
...
<div style={{ backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
  ...
  {/* Gesamtgewicht Eingabe wenn Modus = gesamt */}
  ...
  {/* Magic Input für Copy/Paste UND Upload */}
  <div style={{ 
    marginTop: '1rem', 
    padding: '20px', 
    ...
    {/* Tab-Navigation für Input-Methoden */}
    <div style={{
      display: 'flex',
      gap: '8px',
      marginBottom: '16px',
...
onBlur={(e) => {
  ...
  // 1. PACKSTÜCKE ERKENNEN (VEREINFACHT)
  ...
  // 3. AIR FREIGHT RATE ERKENNEN
  const airFreightMatch = text.match(/Air\s*freight[:\s]*\$?([\d.]+)\/kg/i);
  if (airFreightMatch) {
    console.log('✅ Air Freight Rate gefunden:', airFreightMatch[1]);
    const rate = parseFloat(airFreightMatch[1]);
    const totalWeight = formData.colli.reduce((sum, c) => {
      return sum + (parseInt(c.anzahl) || 0) * (parseFloat(c.gewichtProStueck) || 0);
    }, 0);
    
    if (totalWeight > 0) {
      setFormData(prev => ({ 
        ...prev, 
        warenbeschreibung: `Air Freight: $${rate}/kg (Total: $${(rate * totalWeight).toFixed(2)})` 
      }));
    }
    updatesMade = true;
  }
  
  // 4. CUSTOMS CLEARANCE ERKENNEN
  const customsMatch = text.match(/Customs\s*Clearance[:\s]*\$?([\d,]+)/i);
  if (customsMatch) {
    console.log('✅ Customs Clearance gefunden:', customsMatch[1]);
    setFormData(prev => ({ 
      ...prev, 
      sonderanweisungen: `Customs Clearance: $${customsMatch[1]}` 
    }));
    updatesMade = true;
  }
  
  // 5. PARTNER ERKENNEN
  const partnerMatches = text.matchAll(/"([^"]+)"/g);
  const partners = Array.from(partnerMatches).map(match => match[1]);
  if (partners.length > 0) {
    console.log('✅ Partner gefunden:', partners);
    setFormData(prev => ({ 
      ...prev, 
      warenbeschreibung: prev.warenbeschreibung + ` | Partner: ${partners.join(', ')}` 
    }));
    updatesMade = true;
  }
  ...
  if (updatesMade) {
    ...
    // Feedback für nicht erkannte Inhalte
    const feedback = document.createElement('div');
      ...
      {showPartnerKalkulation && (
        <PartnerKalkulation 
          ...
          onClose={() => {
            ...
            // Reload um Anfragen anzuzeigen
            if (window.location.pathname.includes('sendung')) {
              window.location.reload();
            }
          }}
          onComplete={(completeData) => {
            // Nur speichern wenn es KEINE Anfrage ist
